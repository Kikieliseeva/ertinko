...
def close_and_redirect():
    root.destroy()
    # Добавьте здесь код для перенаправления на другую страницу или выполнения других действий

# Создание главного окна
root = tk.Tk()
root.title("Регистрация приюта для собак")
...
# Кнопки для выхода из окна
close_button = tk.Button(root, text="Закрыть окно", command=close_window)
close_button.pack()

redirect_button = tk.Button(root, text="На главную", command=close_and_redirect)
...
try:
    ...
    # Загрузка и изменение размеров изображения background2.jpg
    background2_image = Image.open("background2.jpg")
    background2_image = background2_image.resize((500, 300), Image.ANTIALIAS)
    background2_img = ImageTk.PhotoImage(background2_image)

    # Загрузка и изменение размеров изображения background3.jpg
    background3_image = Image.open("background3.jpg")
    ...
    traceback.print_exc()
    print("Ошибка при загрузке и изменении размеров изображения фона, background2.png и background3.png: